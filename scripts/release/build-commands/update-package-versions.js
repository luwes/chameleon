...
const update = async ({cwd, dry, packages, version}) => {
  try {
    ...
    // Update chameleon version source file
    const sharedVersionPath = join(cwd, 'packages/shared/version.js');
    const sharedVersion = readFileSync(sharedVersionPath, 'utf8').replace(
      /module\.exports = '[^']+';/,
      `module.exports = '${version}';`
    );
    writeFileSync(sharedVersionPath, sharedVersion);
